...
-module(lens).
-export([
    get/2, put/3, update/3, c/2, c/3, c/4, c/5, c/6, complete/2,
    id/0,
    hd/0, tl/0, index/1, where/1, all/1,
    gb_lookup/1, % gb_tree/1,
    integer/0,
    tuple/1,
    pair_fst/0, pair_snd/0,
    triple_fst/0, triple_snd/0, triple_thd/0
]).

-compile({inline, [
    get/2, put/3, update/3, c/2, c/3, c/4, c/5, c/6,
    hd/0, tl/0, index/1, where/1,
    tuple/1,
    pair_fst/0, pair_snd/0,
    triple_fst/0, triple_snd/0, triple_thd/0
]}).
...
set_nth(1, [_|T], X) -> [X|T];
set_nth(N, [H|T], X) when N > 1 ->
    [H|set_nth(N-1, T, X)].

update_nth(1, [X|T], F) -> [F(X)|T];
update_nth(N, [H|T], F) when N > 1 ->
    [H|update_nth(N-1, T, F)].
...
index(N) ->
    { fun (Xs)    -> nth(N, Xs) end
    , fun (Xs, Y) -> set_nth(N, Xs, Y) end
    , fun (Xs, F) -> update_nth(N, Xs, F) end
    }.
...
set_find(P, [H|T], X) ->
    case P(H)
      of true  -> [X|T]
       ; false -> [H|set_find(P, T, X)]
    end.

update_find(P, [H|T], F) ->
    case P(H)
      of true  -> [F(H)|T]
       ; false -> [H|update_find(P, T, F)]
    end.       
...
where(P) ->
    { fun (Xs)    -> find(P, Xs) end
    , fun (Xs, Y) -> set_find(P, Xs, Y) end
    , fun (Xs, F) -> update_find(P, Xs, F) end
    }.
...
c({G1,P1,U1}, {G2,P2,U2}, {G3,P3,U3}, {G4,P4,U4}) ->
...
c({G1,P1,U1}, {G2,P2,U2}, {G3,P3,U3}, {G4,P4,U4}, {G5,P5,U5}) ->
...
c({G1,P1,U1}, {G2,P2,U2}, {G3,P3,U3}, {G4,P4,U4}, {G5,P5,U5}, {G6,P6,U6}) ->
